...
SELECT CONCAT('SELECT ',STRING_AGG(CONCAT(CAST(NULL AS VARCHAR(MAX)), QUOTENAME(REPLACE(name,'''',''''''))),', ') WITHIN GROUP(ORDER BY NAME), ' FROM [dbo].[Houseamp_WA_OL_Chg]') FROM sys.[dm_exec_describe_first_result_set]('SELECT * FROM [dbo].[Houseamp_WA_OL_Chg]',NULL,NULL)
...
SELECT
  ...
  , [t].[IsDate_SQL]
  , [t].[YNTF_SQL]
  INTO [Profile].[Houseamp_WA_OL_Chg]
